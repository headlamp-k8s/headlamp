...
export async function fetchAndExecutePlugins(
  ...
  // If a plugin was added or removed,
  //  then packageInfos would not be the same length as settingsPackages
  // Update settingsPackages:
  // - Add missing plugins
  // - Remove plugins that are not in packageInfos anymore.

  const updatedSettingsPackages =
    packageInfos.length !== settingsPackages.length
      ? updateSettingsPackages(packageInfos, settingsPackages)
      : settingsPackages;

  if (packageInfos.length !== settingsPackages.length) {
