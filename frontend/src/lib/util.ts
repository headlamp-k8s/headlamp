...
type URLStateParams<T> = {
  ...
  defaultValue: T;
  /** Whether to add 1 to the value when setting it to the URL, and to subtract 1 when getting the value from
   * the URL. This is useful for when the URL state is used for a page number, and the page number is 1-indexed
   */
...
export function useURLState<T extends string | number | undefined = string>(
  ...
  const { defaultValue, isOneIndexed = false, prefix = '' } = params;
  ...
  React.useEffect(() => {
    ...
    if (value === null || value === defaultValue) {
      ...
      let urlValue = value as NonNullable<T>;
      if (typeof value === 'number' && isOneIndexed) {
        urlValue = (value + 1) as NonNullable<T>;
      }
