...
export interface SidebarState {
   ...
   */
  entries: { [propName: string]: SidebarEntryProps };
  /**
   * Filters to apply to the sidebar entries.
   */
  filters: ((entry: SidebarEntryProps) => SidebarEntryProps | null)[];
}
...
const sidebarSlice = createSlice({
  ...
  reducers: {
     ...
     */
    setSidebarItem(state, action: PayloadAction<SidebarEntryProps>) {
    ...
    setSidebarItemFilter(
      state,
      action: PayloadAction<(entry: SidebarEntryProps) => SidebarEntryProps | null>
    ) {
