...
import { Edge, Node, Panel, ReactFlowProvider, useReactFlow, useStore } from '@xyflow/react';
import {
  createContext,
  ReactNode,
  StrictMode,
...
function GraphViewContent({
  ...
  useEffect(() => {
    applyGraphLayout(visibleGraph, aspectRatio).then(layout => {
      ...
      setTimeout(() => {
        const bounds = flow.getNodesBounds(layout.nodes);
