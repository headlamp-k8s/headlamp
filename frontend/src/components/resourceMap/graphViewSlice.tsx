...
export interface DetailsPageDefinition {
  /**
   * React component that will render the details
   * @param props.name - Name of the resource
   * @param props.namespace - Namespace of the resource (optional)
   * @returns Details Page
   */
  component: (props: { name: string; namespace?: string }) => ReactNode;
}

interface GraphViewSliceState {
  graphSources: GraphSource[];
  kindIcons: Record<string, IconDefinition>;
  kindDetailsPage: Record<string, DetailsPageDefinition>;
}
...
export const graphViewSlice = createSlice({
  ...
  reducers: {
    ...
    addKindDetailsPage(
      state,
      action: PayloadAction<{ kind: string; definition: DetailsPageDefinition }>
    ) {
      state.kindDetailsPage[action.payload.kind] = action.payload.definition;
    },
  },
});
