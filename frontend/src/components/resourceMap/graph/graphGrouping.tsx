...
const getConnectedComponents = (nodes: KubeObjectNode[], edges: GraphEdge[]): GraphNode[] => {
  ...
  const dfs = (node: KubeObjectNode, componentNodes: KubeObjectNode[]) => {
...
export function groupGraph(
    ...
    components = groupByProperty(
      ...
      component => {
        ...
        if (component.type === 'group') {
          return null; // TODO: handle grouping of groups
        }
  ...
  if (groupBy === 'node') {
    components = groupByProperty(
      ...
      component => {
        if (component.type === 'kubeGroup') {
          const maybePod = component.data.nodes.find(it => it.data.resource?.kind === 'Pod');
          return maybePod?.data.resource?.spec?.nodeName;
        }
        if (component.type === 'group') {
          return null; // TODO: handle grouping of groups
        }
