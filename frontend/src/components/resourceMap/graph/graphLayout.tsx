...
function getPartitionLayer(node: KubeObjectNode) {
  let layer: any = undefined;

  if ('resource' in node.data) {
    const res = node.data.resource;
    if (res.kind === 'Deployment') {
      layer = 1;
    }
    if (res.kind === 'ReplicaSet' || res.kind === 'ServiceAccount' || res.kind === 'CronJob') {
      layer = 3;
    }
    ...
    if (res.kind === 'Pod' || res.kind === 'RoleBinding') {
      layer = 5;
    }
    if (res.kind === 'Service' || res.kind === 'NetworkPolicy' || res.kind === 'Role') {
      layer = 6;
    }
  }
...
function convertToElkNode(node: GraphNode, aspectRatio: number): ElkNodeWithData {
  ...
  if (node.type === 'kubeObject') {
    elkNode.layoutOptions = {
      'partitioning.partition': getPartitionLayer(node),
    };
