...
func (c *HeadlampConfig) addProxyForContext(context *Context, r *mux.Router) {
	...
	// if they exist.
	clientCert := context.authInfo.ClientCertificate
	clientKey := context.authInfo.ClientKey
	if clientCert != "" {
		if clientKey == "" {
			log.Fatal("Found a ClientCertificate entry for cluster %s, but not a ClientKey.", name)
		} else {
	...
	clientCertData := context.authInfo.ClientCertificateData
	clientKeyData := context.authInfo.ClientKeyData
	if clientCertData != nil {
		if clientKeyData == nil {
			log.Fatal("Found a ClientCertificateData entry for cluster %s, but not a ClientKeyData.", name)
		} else {
