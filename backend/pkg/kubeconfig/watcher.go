	...
	for {
		...
		case <-ticker.C:
			if len(watcher.WatchList()) != len(kubeConfigPaths) {
				log.Println("watcher is missing some files, re-adding them")
				addFilesToWatcher(watcher, kubeConfigPaths)
				LoadAndStoreKubeConfigs(kubeConfigStore, paths)
			}
		...
		case event := <-watcher.Events:
			...
			for _, trigger := range triggers {
				...
				if event.Op.Has(trigger) {
					log.Println("kubeconfig file changed, reloading contexts")
					LoadAndStoreKubeConfigs(kubeConfigStore, paths)
				}
			}

		case err := <-watcher.Errors:
			log.Println("error watching kubeconfig files", err)
		}
	}
}
